...
class TransactionFetcher extends _Model.ModelFetcher {
  ...
  genOrderedCollectionForAccountLink(accountLinkID, limit = 20) {
    ...
    return _asyncToGenerator(function* () {
      let query = _this.__firebaseCollection.where('accountLinkRef.refID', '==', accountLinkID).orderBy('transactionDate', 'desc');
      if (limit !== Infinity) {
  ...
  genOrderedCollectionForAccount(accountID, limit = 20) {
    ...
    return _asyncToGenerator(function* () {
      let query = _this2.__firebaseCollection.where('accountRef.refID', '==', accountID)
  ...
  genForYodleeTransaction(yodleeTransaction) {
    ...
    return _asyncToGenerator(function* () {
      const snapshot = yield _this3.__firebaseCollection.where('sourceOfTruth.type', '==', 'YODLEE').where('sourceOfTruth.value.id', '==', yodleeTransaction.id).get();
