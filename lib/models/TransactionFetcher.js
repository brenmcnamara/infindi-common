...
class TransactionFetcher extends _Model.ModelFetcher {
  ...
  genOrderedCollectionForUser(userID, limit = 20) {
    ...
    return _asyncToGenerator(function* () {
      let query = _this.__firebaseCollection.where('userRef.refID').orderBy('transactionDate', 'desc');
      if (limit !== Infinity) {
  ...
  genOrderedCollectionForAccountLink(accountLinkID, limit = 20) {
    ...
    return _asyncToGenerator(function* () {
      let query = _this2.__firebaseCollection.where('accountLinkRef.refID', '==', accountLinkID).orderBy('transactionDate', 'desc');
      if (limit !== Infinity) {
        query = query.limit(limit);
      }
      const snapshot = yield query.get();
