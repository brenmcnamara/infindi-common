...
exports.genUpdateAccountLink = genUpdateAccountLink;
...
function createAccountLink(sourceOfTruth, userID, providerID, providerName) {
...
function createAccountLinkYodlee(yodleeProviderAccount, userID, providerID, providerName) {
  const sourceOfTruth = {
...
function updateAccountLinkStatus(accountLink, status) {
  const now = new Date();
  const sourceOfTruth = accountLink.sourceOfTruth.type === 'YODLEE' && status === 'MFA / WAITING_FOR_LOGIN_FORM' ? {
    loginForm: null,
...
function genUpdateAccountLink(accountLink) {
