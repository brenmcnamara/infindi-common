...
function isLinking(accountLink) {
  return accountLink.status.startsWith('IN_PROGRESS') && accountLink.status !== 'IN_PROGRESS / USER_INPUT_REQUIRED';
}
...
function calculateAccountLinkStatus(sourceOfTruth) {
  ...
  if (refreshInfo.status === 'IN_PROGRESS') {
    return refreshInfo.additionalStatus === 'LOGIN_IN_PROGRESS' ? 'IN_PROGRESS / VERIFYING_CREDENTIALS' : refreshInfo.additionalStatus === 'USER_INPUT_REQUIRED' ? 'IN_PROGRESS / USER_INPUT_REQUIRED' : 'IN_PROGRESS / DOWNLOADING_DATA';
  }
