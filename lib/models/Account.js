...
exports.getAccountCollection = getAccountCollection;
exports.createAccountYodlee = createAccountYodlee;
exports.updateAccountYodlee = updateAccountYodlee;
exports.genFetchAccount = genFetchAccount;
exports.genFetchAccountsForAccountLink = genFetchAccountsForAccountLink;
...
function createAccountYodlee(yodleeAccount, accountLinkID, userID, isTestAccount = false) {
...
function updateAccountYodlee(account, yodleeAccount) {
  // $FlowFixMe - Ignore this for now.
  return _extends({}, (0, _dbUtils.updateModelStub)(account), {
    sourceOfTruth: {
      type: 'YODLEE',
      value: yodleeAccount
    }
...
function genFetchAccount(accountID) {
  return getAccountCollection().doc(accountID).get().then(doc => doc.exists ? doc.data() : null);
}

function genFetchAccountsForAccountLink(accountLinkID) {
  return getAccountCollection().where('accountLinkRef.refID', '==', accountLinkID).get().then(snapshot => snapshot.docs.filter(doc => doc.exists).map(doc => doc.data()));
}

function genCreateAccount(account) {
  return getAccountCollection().doc(account.id).set(account);
}

function genUpdateAccount(account) {
  return getAccountCollection().doc(account.id).update(account);
}
