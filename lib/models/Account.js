...
exports.genFetchAccountsForRefreshInfo = undefined;

var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };

let genFetchAccountsForRefreshInfo = exports.genFetchAccountsForRefreshInfo = (() => {
  var _ref = _asyncToGenerator(function* (refreshInfoID) {
    const refreshInfo = yield (0, _RefreshInfo.genFetchRefreshInfo)(refreshInfoID);
    if (!refreshInfo) {
      return [];
    }
    const snapshot = yield getAccountsCollection().where('sourceOfTruth.value.providerId', '==', refreshInfo.providerRef.refID).get();
  ...
  return function genFetchAccountsForRefreshInfo(_x) {
...
exports.updateAccountFromYodleeAccount = updateAccountFromYodleeAccount;
...
var _RefreshInfo = require('./RefreshInfo');
...
function updateAccountFromYodleeAccount(account, yodleeAccount) {
