...
function createRefreshSchedule(refreshInfo) {
  ...
  if (yodleeRefreshInfo.nextRefreshScheduled) {
    const runAtUpperBound = new Date(Date.parse(yodleeRefreshInfo.nextRefreshScheduled));
    const runAtOneDay = new Date(Date.now() + MILLIS_PER_DAY * 1.0);
    return {
      recurringType: 'ONCE',
      runAt: runAtUpperBound.getTime() < runAtOneDay.getTime() ? runAtUpperBound : runAtOneDay
    };
  }
